...
function NavBar(el, page) {
  ...
  this.menuToggler.addEventListener('click', function(event) {
  ...
  this.page.sections.forEach(function(section, index) {
    ...
    a.addEventListener('click', function() {
      ...
      document.documentElement.scrollTop = section.el.offsetTop - this.el.offsetHeight + 1;
      this.closeMenu();
...
Project.prototype.addScrollMagicScenesForHorizontalToSection = function() {
  ...
  this.itemEl.querySelectorAll('.blahlab-block-portfolio-item-image.vertical').forEach(function(portfolioItemImage, index) {
    ...
    let translateY = [0, portfolioItemImage.offsetHeight - image.offsetHeight];
...
Project.prototype.removeScrollMagicScenesForHorizontalFromSection = function() {
  this.scenes.horizontal.forEach(function(scene) {
...
Project.prototype.fetchData = function() {
  ...
  return fetch(this.link.href).then( response => response.text() ).then(function(data) {
    ...
    // for horizontal
    this.itemEl = parser.parseFromString(this.data, 'text/html').querySelector('.pagesection__content');
    ...
    this.goBackLink.addEventListener('click', function(event) {
      event.preventDefault();
      this.hideItem();
    }.bind(this));

    this.itemEl.addEventListener('wheel', function(event) {
    ...
    // for vertical
    this.itemElVertical = parser.parseFromString(this.data, 'text/html').querySelector('.pagesection__content');
    this.itemElVertical.style.display = 'none';
...
function Section(el, page) {
  ...
  this.mediaQueryForHorizontalMode.addListener(this.handleMediaQueryChange.bind(this));
...
Section.prototype.onResize = function() {
  ...
  this.onResizeTimeout = setTimeout(function() {
    ...
    this.page.sections.forEach(function(section) {
      section.content.style.flexBasis = this.page.activatedSection.el.offsetWidth + 'px';
    }.bind(this))
...
Section.prototype.onScroll = function(event) {
  ...
  if ( Math.ceil(this.content.scrollLeft) >= this.content.scrollWidth - this.content.clientWidth && forward ) {
    this.goToNext();
  }
...
Section.prototype.setupScrollMagicScenesForVertical = function() {
  ...
  let scene = new ScrollMagic.Scene({
    ...
    triggerHook: 'onLeave',
    offset: -49,
    duration: (function() { return this.content.offsetHeight }).bind(this),
  })
...
Section.prototype.setupScrollMagicScenesForHorizontal = function() {
  ...
  this.el.querySelectorAll('.blahlab-block-team-members').forEach(function(teamMembersBlock, index) {
    ...
    if ( nextSection ) {
      ...
      teamMembersBlockAndNextSection.classList.add('blahlab-block-team-members-and-sibling');

      teamMembersBlockAndNextSection.style.width = teamMembersBlock.offsetWidth + nextSection.offsetWidth + 'px';
    ...
    let teamMembersBlockPaddingTop = parseInt(getComputedStyle(teamMembersBlock)['padding-top']) || 0;
    let pagesectionContent = teamMembersBlock.closest('.pagesection__content');
    let pagesectionContentPaddingTop = parseInt(getComputedStyle(pagesectionContent)['padding-top']) || 0;
    let totalPaddingTop = pagesectionContentPaddingTop + teamMembersBlockPaddingTop;
    ...
    let memberMarginBottom = parseInt(getComputedStyle(member)['margin-bottom']) || 0;

    let translateY = [0, -( totalPaddingTop + memberHeight * members.length + memberMarginBottom * (members.length - 1)  - window.innerHeight )];
  ...
  this.el.querySelectorAll('.blahlab-block-picture').forEach(function(pictureBlock, index) {
  ...
  this.el.querySelectorAll('.blahlab-block-projects__project').forEach(function(projectBlock, index) {
  ...
  this.el.querySelectorAll('.blahlab-block-blog-posts__post').forEach(function(postBlock, index) {
...
Section.prototype.removeScrollMagicScenesForHorizontal = function() {
  ...
  setTimeout(function() {
    // remove .blahlab-block-team-members-and-sibling and restore its children
    let wrappers = document.querySelectorAll('.blahlab-block-team-members-and-sibling');
...
function init() {
  ...
  sectionElements.forEach(function(sectionElement, index) {
    ...
    if ( section.el.querySelector('.blahlab-block-projects') ) {

      // initialize projects here
      section.el.querySelectorAll('.blahlab-block-projects__project').forEach(function(projectElement, index) {
      ...
      if ( page.sections[0].mode == 'horizontal' ) {
        ...
        loadingAnimationTimeline.add({
          ...
          complete: function(anim) {
            ...
            document.querySelectorAll('.pagesection').forEach(function(pageSection) {
              ...
              anime({
                ...
                complete: function(anim) {
                  ...
                  document.querySelectorAll('.pagesection__content').forEach(function(pageContent) {
                    pageContent.style.visibility = 'visible';
                  });
                  document.querySelector('#logo').style.opacity = 1;
                  document.querySelector('#menu-toggler').style.opacity = 1;
                  document.querySelectorAll('.pagesection__title span').forEach(function(span) {
        ...
        loadingAnimationTimeline.add({
          ...
          complete: function(anim) {
            ...
            document.querySelectorAll('.pagesection').forEach(function(pageSection, index) {
        ...
        loadingAnimationTimeline.add({
          ...
          begin: function(anim) { 
            pageSections.forEach(function(section, index) {
              section.querySelector('.pagesection__content').style.visibility = 'visible';
              section.querySelector('.pagesection__content').style.height = 'auto';
              section.querySelector('.pagesection__title').style.borderBottom = '1px solid #000';
            });
          },
          complete: function(anim) {
            // unset all styles set during loading
            pageSections.forEach(function(pageSection, index) {
              ...
              pageSection.querySelector('.pagesection__title').style.borderBottom = ''
